export function buildCsvSummary(csvData: Record<string, Record<string, unknown>[]>): string {
  ...
  if (csvData.content && csvData.content.length > 0) {
    ...
    const totalSubscribersGained = rows.reduce((sum, row) => sum + toNumber(row['Subscribers'] ?? row['subscribers']), 0)
    const totalImpressions = rows.reduce((sum, row) => sum + toNumber(row['Impressions'] ?? row['impressions']), 0)
    ...
    if (topVideo) {
      ...
      const topWatchHours = toNumber(topVideo['Watch time (hours)'] ?? topVideo['watch time (hours)'])
      const topCtr = toNumber(topVideo['Impressions click-through rate (%)'] ?? topVideo['impressions click-through rate (%)'])
      parts.push(
  ...
  if (csvData.geography && csvData.geography.length > 0) {
    const rows = csvData.geography.filter(r => String(r['Geography'] ?? '').trim().toUpperCase() !== 'TOTAL')
    const totalViews = rows.reduce((sum, r) => sum + toNumber(r['Views']), 0)

    const tier1Codes = new Set(['US', 'GB', 'CA', 'AU', 'NZ'])
    const tier1Views = rows
      .filter(r => tier1Codes.has(String(r['Geography'] ?? '').trim().toUpperCase()))
      .reduce((sum, r) => sum + toNumber(r['Views']), 0)
    ...
    const topCountries = rows
      ...
      .slice(0, 5)
      .map(r => {
        const geo = String(r['Geography'] ?? '')
        const views = toNumber(r['Views'])
  ...
  if (csvData.traffic_sources && csvData.traffic_sources.length > 0) {
    const rows = csvData.traffic_sources.filter(r => String(r['Traffic source'] ?? '').trim().toLowerCase() !== 'total')
    const totalViews = rows.reduce((sum, r) => sum + toNumber(r['Views']), 0)
    ...
    const topSources = rows
      ...
      .slice(0, 4)
      .map(r => {
        const source = String(r['Traffic source'] ?? '')
        const views = toNumber(r['Views'])
        const pct = totalViews > 0 ? ` ${((views / totalViews) * 100).toFixed(0)}%` : ''
        const ctr = toNumber(r['Impressions click-through rate (%)'])
        return `${source}${pct}${ctr > 0 ? ` (${ctr}% CTR)` : ''}`
      })
      .join(', ')

    const searchViews = rows
      .filter(r => String(r['Traffic source'] ?? '').toLowerCase().includes('search'))
      .reduce((sum, r) => sum + toNumber(r['Views']), 0)
