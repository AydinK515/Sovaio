...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  let latestRateCard: RateCard | null = null
  ...
  if (user) {
    const [
      ...
      supabase.from('rate_cards').select('*').eq('user_id', user.id).order('created_at', { ascending: false }).limit(1).maybeSingle(),
    ...
    latestRateCard = (rateCardResponse.data as RateCard | null) ?? null
